...
Then ('user click on HTS Retest Form', () => {  
    cy.contains('HTS Retest Form').click();
});
...
And ('the user enters the details for Test One Mishima', () => {
    ...
    cy.get('input[id="w138_0"]').click();
    cy.get('input[name="w139"]').type(details.lotNumber2, {force: true});

 
    cy.get('textarea[name="w143"]').type(details.remarks, {force: true});
